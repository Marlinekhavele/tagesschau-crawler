...
class Crawler:
	def __init__(self, base_url=None, overview_url=None):
		self.base_url = base_url or current_app.config.get('TAGESSCHAU_BASE_URL')
		self.overview_url = overview_url or current_app.config.get('TAGESSCHAU_OVERVIEW_URL')
		self.headers = {
	...
	def crawl_overview_page(self):
		...
		try:
			...
			# Find all article links - adjust selectors based on actual page structure
			article_links = []
